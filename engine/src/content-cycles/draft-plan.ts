 ...
 *
 * The worker's own implementation rather than a call into the app's approveDraft: the two
 * live in different packages and the worker cannot import from app/. The RULES are kept
 * identical on purpose — atomic transition, refuse if already approved, stamp
 * approved_by='auto' — and both have their own tests. If these ever diverge, that is a bug
 * in whichever one is wrong, not a design. Recorded in the report as the one real
 * duplication this build introduces.
 *
 * Enqueues one shape job per beat exactly as the client-approved path does, so a month
 * that went ahead automatically is generated by the same machinery as one someone pressed
 * a button for.
 */
export async function autoApproveAndGenerate(
  ...
  const [cycle] = await db
    .select({ id: contentCycles.id, channel: contentCycles.channel, approvedAt: contentCycles.approvedAt })
    .from(contentCycles)
    .where(and(eq(contentCycles.id, cycleId), eq(contentCycles.clientId, clientId)))
    .limit(1);
  if (!cycle) return { approved: 0, captionsQueued: 0 };
  // Already approved — the client got there first. Nothing to do, and certainly not a
  // second fan-out.
  if (cycle.approvedAt) {
    logger.info({ cycleId }, 'draft-plan: cutoff reached but the client already approved — no auto-approval');
  ...
  if (drafts.length === 0) return { approved: 0, captionsQueued: 0 };
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.update(contentCycles)
      .set({ approvedAt: now, approvedBy: 'auto', updatedAt: now })
  ...
  for (const post of drafts) {
    ...
    try {
      await queue.add('shape', {
        ...
        source: 'web',
      }, { jobId: `shape_${cycleId}_${post.id}`, attempts: 1, removeOnComplete: { age: 3600, count: 1000 }, removeOnFail: { age: 3600 } });
      captionsQueued++;
      if (post.format === 'reel' || post.format === 'carousel') {
        await queue.add('hook', { type: 'hook', clientId, cycleId, targetPostId: post.id },
          { jobId: `hook_${cycleId}_${post.id}`, attempts: 1, removeOnComplete: { age: 3600, count: 1000 }, removeOnFail: { age: 3600 } });
      }
    } catch (err) {
  ...
  logger.info({ cycleId, approved: drafts.length, captionsQueued }, 'draft-plan: auto-approved at cutoff and started phase 2');
  return { approved: drafts.length, captionsQueued };
}
