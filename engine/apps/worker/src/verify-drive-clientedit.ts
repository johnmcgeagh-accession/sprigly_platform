...
if (phase === 'setup') {
  ...
  try {
    ...
    console.log('');
    console.log('  Make ANY change that Drive will record as a modification:');
    console.log('');
    console.log('  OPTION A — rename (simplest, no download needed):');
    console.log('    Right-click the file → Rename → append "-edited" → press Enter');
    console.log('');
    console.log('  OPTION B — replace content (closer to production xlsx replace):');
    console.log('    Right-click → Manage versions → Upload new version → choose any file');
    console.log('');
