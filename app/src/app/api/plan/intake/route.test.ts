...
vi.mock('drizzle-orm', () => ({ and: (...p: unknown[]) => ({ op: 'and', p }), eq: (c: unknown, v: unknown) => ({ op: 'eq', c, v }) }));
vi.mock('@sprigly/db', () => ({
  db: {
    select: () => ({ from: () => ({ where: () => ({ limit: () => Promise.resolve(h.cycleRow) }) }) }),
...
beforeEach(() => {
  ...
  h.cycleRow = [{ status: 'requested', intakeJson: null }];
...
describe('POST /api/plan/intake — classifier', () => {
  ...
  it('PRE-cutoff: merges answers (overwrite) + appends freeNotes, and clears the brief', async () => {
    h.cycleRow = [{ status: 'requested', intakeJson: { planContent: { answers: { q1: 'old' }, freeNotes: 'note1' }, businessContext: [], otherChannel: {}, source: 'manual', capturedAt: 'x' } }];
