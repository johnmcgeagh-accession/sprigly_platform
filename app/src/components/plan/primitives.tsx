...
/** Bottom sheet (mobile pattern + desktop agent). `heightClass` sizes it. */
export function Sheet({
  show, onClose, children, heightClass = '', className = '', testid = 'sheet', labelledBy,
}: {
  show: boolean; onClose: () => void; children: React.ReactNode;
  heightClass?: string; className?: string; testid?: string; labelledBy?: string;
}) {
  return (
    <div
      role="dialog" aria-modal="true" aria-labelledby={labelledBy} data-testid={testid}
      className={[
        'fixed inset-x-0 bottom-0 z-[61] flex flex-col rounded-t-[26px] bg-surface shadow-sheet',
        'transition-transform duration-[420ms] ease-sheet',
        heightClass, className,
        show ? 'translate-y-0' : 'translate-y-full',
      ].join(' ')}
    >
      <div className="mx-auto mt-2.5 h-[5px] w-11 flex-none rounded-full bg-[#E2DED9]" />
...
/** Right drawer (desktop editor). Slides from the right; plan stays visible behind. */
export function Drawer({
  show, onClose, children, testid = 'drawer',
}: { show: boolean; onClose: () => void; children: React.ReactNode; testid?: string }) {
  return (
    <div
      role="dialog" aria-modal="true" data-testid={testid}
      className={[
        'fixed inset-y-0 right-0 z-[61] flex w-[min(560px,46vw)] flex-col border-l border-line bg-surface',
        'shadow-[-24px_0_60px_-18px_rgba(27,36,48,0.3)] transition-transform duration-[400ms] ease-sheet',
        show ? 'translate-x-0' : 'translate-x-[103%]',
      ].join(' ')}
    >
...
/** Transient status pill, bottom-centre. */
...
/** Two-way segmented control (Plan | Tasks) with a sliding pill. */
export function SegmentedControl<T extends string>({
  ...
  useEffect(() => {
    ...
    if (!w || !p) return;
    const active = w.querySelector<HTMLButtonElement>(`[data-on="true"]`);
    if (!active) return;
  ...
  return (
    <div ref={wrap} data-testid="segmented" className={`relative inline-flex rounded-full bg-[#ECEAE6] p-1 ${className}`}>
      <span ref={pill} className="absolute inset-y-1 left-1 z-[1] rounded-full bg-surface shadow-card transition-[transform,width] duration-300 ease-sheet" />
      {options.map((o) => (
        <button
          key={o.value} data-on={value === o.value} data-testid={`seg-${o.value}`}
        ...
        >
          {o.label}
          {o.dot && <span className="inline-block h-1.5 w-1.5 rounded-full bg-amber-500" />}
        </button>
      ))}
    </div>
  );
