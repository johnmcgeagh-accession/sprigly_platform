...
export function ProgressRing({ done, total, risk, size = 34 }: { done: number; total: number; risk?: boolean; size?: number }) {
  ...
  const num = complete ? 'text-coral' : risk ? 'text-[#B77400]' : 'text-slate-600';
  return (
    <span data-testid="progress-ring" className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
...
export function PostChip({ post, selected, today, onClick, draggable, onDragStart, onDragEnd }: {
  ...
  return (
    ...
    >
      ...
      {post.status === 'new'
        ? <span className="mt-0.5 flex-none rounded-[4px] border border-coral px-[3px] text-[8.5px] font-extrabold tracking-[.06em] text-coral">NEW</span>
...
export function ChecklistItem({ step, scheduledDate, today, onToggle, testid }: {
  ...
  return (
    <div data-testid={testid ?? 'checklist-item'} className={[
      ...
      <span className={`whitespace-nowrap text-[11.5px] font-extrabold ${late ? 'text-[#C0553A]' : isToday ? 'text-coral' : 'text-muted'}`}>{label}</span>
    </div>
  );
}
  ...
  return (
    <div data-testid="proposal-card" data-proposal-id={proposal.id}
      ...
      <div className="flex flex-none gap-2">
        <button data-testid="proposal-approve" disabled={busy} onClick={onApprove}
        ...
        <button data-testid="proposal-discard" disabled={busy} onClick={onDiscard}
    ...
    <div data-testid="extraction-summary" className="mt-[18px] rounded-[14px] border border-line bg-[#FAF9F7] px-4 py-3.5">
      ...
      {reply.proposals.map((p) => (
        <div key={p.id} className="flex items-start gap-2.5 py-1.5 text-[13.5px] font-bold text-slate-700">
          ...
          <span className="mt-1 whitespace-nowrap text-[11px] font-bold text-coral">→ Approvals</span>
        </div>
      ))}
