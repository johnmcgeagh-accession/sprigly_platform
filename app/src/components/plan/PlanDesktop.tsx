...
export function PlanDesktop({ data }: { data: PlanData }) {
  ...
  const railBtn = (v: View, label: string, Icon: React.FC<{ className?: string }>, count: number, warn?: boolean, dot?: boolean) => (
    <button
      data-testid={`nav-${v}`} onClick={() => setView(v)} title={label}
      className={[
        'relative flex w-full items-center gap-[11px] rounded-xl px-3 py-[11px] text-left text-[14.5px] font-bold',
        railCollapsed ? 'justify-center px-0' : '',
        view === v ? 'bg-coral-tint text-[#B5503F]' : 'text-slate-600 hover:bg-line-soft',
      ].join(' ')}
    >
      ...
      {!railCollapsed && count > 0 && (
        <span className={`ml-auto rounded-full px-2 py-px text-[11px] font-extrabold ${warn ? 'bg-amber-tint text-[#B77400]' : view === v ? 'bg-white text-slate-600' : 'bg-[#ECEAE6] text-slate-600'}`}>{warn ? `${count} late` : count}</span>
      )}
  ...
  return (
    <div className="flex h-screen flex-col bg-bg text-slate-700" data-testid="plan-desktop">
      ...
      <header className="flex h-[66px] flex-none items-center justify-between border-b border-line bg-surface px-7">
        <div className="flex items-center gap-2.5"><SprigMark className="h-[26px] w-[26px]" /><span className="text-[22px] font-extrabold tracking-tight text-coral">Sprigly</span></div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside data-testid="rail" className={`order-2 flex flex-none flex-col gap-1.5 overflow-hidden border-l border-line bg-surface px-3.5 py-[18px] transition-[width] duration-300 ease-sheet ${railCollapsed ? 'w-[72px] px-3' : 'w-[224px]'}`}>
          <div className="mb-2.5 flex min-h-[40px] items-center justify-between gap-2">
            {!railCollapsed && <div className="font-serif text-[19px] leading-tight text-slate-700">{data.clientName} · <em className="not-italic text-coral">{MONTHS[month]}</em></div>}
      ...
      <Sheet show={agentOpen} onClose={() => setAgentOpen(false)} testid="agent-sheet">
        <div className="mx-auto max-h-[82vh] w-full max-w-[940px] overflow-y-auto px-9 pb-9 pt-2">
          <div className="mb-1 mt-1.5"><div className="text-[11px] font-extrabold uppercase tracking-[.14em] text-muted">Plan agent</div><div className="font-serif text-[27px] text-slate-700">Talk to your <em className="italic text-coral">plan</em></div></div>
          <p className="mb-5 mt-1 max-w-[560px] text-[14px] font-semibold leading-snug text-muted">Ask in plain English. Sprigly proposes the change and <b className="text-coral">nothing happens until you approve it</b>.</p>
          <div className="flex items-center gap-2.5 rounded-2xl border-[1.5px] border-line bg-surface py-2 pl-[18px] pr-2 focus-within:border-coral">
            <input data-testid="agent-input" value={agentText} onChange={(e) => setAgentText(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter' && agentText.trim()) { void data.ask(agentText, selId); setAgentText(''); } }}
            ...
            <button data-testid="agent-send" disabled={!agentText.trim() || data.agentBusy} onClick={() => { void data.ask(agentText, selId); setAgentText(''); }}
          ...
          <div className="mt-4 text-[12.5px] font-semibold text-muted">Suggestions land in <b className="text-coral">Approvals</b> in the menu — nothing changes until you approve it there.</div>
        </div>
      </Sheet>
    </div>
  );
        ...
        {lead > 0 && (
          <div className="flex items-start rounded-2xl border border-dashed border-[#E6E2DD] p-4" style={{ gridColumn: `span ${lead}` }} data-testid="month-summary">
            <div>
              ...
              <div className={`mt-1 text-[12.5px] font-bold ${riskN ? 'text-[#B77400]' : 'text-muted'}`}>{riskN ? `${riskN} behind schedule — see Tasks` : 'On track'}</div>
            </div>
          </div>
        )}
        {Array.from({ length: dim }, (_, i) => i + 1).map((day) => {
          ...
          return (
            <div key={day} data-testid="calendar-cell" data-date={isoOf(day)}
              ...
              <div className={`px-[3px] py-px text-[13px] font-extrabold ${isToday ? 'text-coral' : wknd ? 'text-muted' : 'text-slate-600'}`}>{day}</div>
              ...
              {!data.readOnly && dayPosts.length === 0 && (
                <button data-testid="add-on-day" onClick={() => data.addPost(isoOf(day))}
                  className="mt-auto rounded-[9px] border-[1.5px] border-dashed border-[#E9E5DF] py-0.5 text-center text-[15px] font-bold text-[#CFC9C1] hover:border-[#EFC9BF] hover:bg-[#FFF9F7] hover:text-coral">＋</button>
              )}
            </div>
          );
        })}
      ...
      {ordered.map((p) => {
        ...
        return (
          <React.Fragment key={p.id}>
            {divider && <div className="grid grid-cols-[40px_1fr] items-center gap-[18px] py-2"><span className="justify-self-center bg-bg px-2 py-0.5 text-[10.5px] font-extrabold uppercase tracking-[.1em] text-coral">Today</span><span className="h-0.5 rounded bg-coral opacity-50" /></div>}
            <div data-testid="timeline-item" onClick={() => onSelect(p.id)} className="grid cursor-pointer grid-cols-[40px_1fr] gap-[18px] py-1.5">
              ...
              <div className={`rounded-2xl border px-4 py-[11px] transition ${p.id === selId ? 'border-line bg-surface shadow-card' : 'border-transparent hover:border-line hover:bg-surface hover:shadow-card'}`}>
                <div className="flex items-center gap-2.5"><span className="font-serif text-[19px] text-slate-700">{monthDayLabel(p.date)}</span>
                  {p.status === 'new' ? <span className="rounded-[5px] bg-coral-tint px-1.5 py-px text-[9px] font-extrabold tracking-[.06em] text-coral">NEW</span> : p.status === 'edited' && <span className="rounded-[5px] border border-line px-1.5 py-px text-[9px] font-bold tracking-[.06em] text-muted">EDITED</span>}
                </div>
  ...
  return (
    <div className="mx-auto max-w-[820px]" data-testid="tasks-board">
      <div className="my-2 flex items-center justify-between rounded-2xl border border-line bg-surface px-[22px] py-[18px] shadow-card">
        ...
        <div className="text-right text-[13px] font-extrabold text-coral">{doneStepCount(data.posts)} done<small className="mt-0.5 block text-[11px] font-bold text-muted">across {data.posts.length} posts</small></div>
      </div>
      {sections.map(([key, label, items]) => items.length > 0 && (
        <div key={key}>
          <div className="flex items-center gap-2.5 px-1 pb-2.5 pt-5"><span className={`text-[12px] font-extrabold uppercase tracking-[.06em] ${key === 'overdue' ? 'text-[#C0553A]' : key === 'week' ? 'text-coral' : 'text-muted'}`}>{label}</span><span className={`rounded-full px-2 py-px text-[11px] font-extrabold ${key === 'overdue' ? 'bg-[#F7E1D7] text-danger' : 'bg-[#ECEAE6] text-slate-600'}`}>{items.length}</span></div>
          {items.map((t) => (
            <div key={t.item.step.id} data-testid="task-row" onClick={() => onSelect(t.item.post.id)}
              ...
              <div className={`whitespace-nowrap text-[11.5px] font-extrabold ${t.bucket === 'overdue' ? 'text-[#C0553A]' : 'text-muted'}`}>{t.bucket === 'overdue' ? 'Late' : `by ${monthDayLabel(t.due).replace(/ \w+ /, ' ')}`}</div>
            </div>
          ))}
        </div>
      ))}
