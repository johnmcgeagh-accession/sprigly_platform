...
function EscalationCard({ item, token, onDone }: ItemCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-red-200 px-6 py-5">
      ...
      <div className="text-xs text-gray-400 mb-4">Category: {item.category}</div>
...
function DraftReplyCard({ item, token, onDone }: ItemCardProps) {
...
const WORKFLOW_DISPLAY_NAMES: Record<string, string> = {
  'sprigly-prospect-research': 'Prospect Research',
  'sprigly-blog-post':         'Blog Post',
  'sprigly-meeting-prep':      'Meeting Prep',
};
...
function WorkflowActionCard({ item, token, onDone }: ItemCardProps) {
  const workflowId = item.suggestedAction.startsWith('invoke_workflow:')
    ? item.suggestedAction.slice('invoke_workflow:'.length)
  ...
  return (
    <div className="bg-white rounded-lg border border-blue-200 px-6 py-5">
      <div className="mb-1">
        <span className="text-sm font-medium text-gray-900">{item.from}</span>
      </div>
      <div className="text-sm text-gray-600 mb-1">{item.subject}</div>
      <div className="text-xs text-gray-400 mb-3">Category: {item.category}</div>
      <div className="text-xs text-blue-700 bg-blue-50 border border-blue-100 rounded px-3 py-2 mb-4">
        Workflow: <span className="font-medium">{workflowDisplayName(workflowId)}</span>
        <span className="text-blue-400 ml-2">({workflowId})</span>
      </div>
      <p className="text-xs text-gray-500 mb-4">
        Approving will run this workflow and deliver output to your inbox.
      </p>
      {error && <p className="text-sm text-red-600 mb-3">{error}</p>}
      <div className="flex items-center gap-3">
        <button
  ...
  return (
    <div className="space-y-8">
        ...
        <section>
          ...
          <div className="space-y-4">
            {escalations.map((item) => (
              <EscalationCard key={item.captureLogId} item={item} token={token} onDone={handleDone} />
            ))}
          </div>
        </section>
      )}
      {workflowActions.length > 0 && (
        <section>
          <h2 className="text-base font-semibold text-gray-900 mb-3">
            Workflow actions — {workflowActions.length}
          </h2>
          <div className="space-y-4">
      ...
      {draftReplies.length > 0 && (
        <section>
          <h2 className="text-base font-semibold text-gray-900 mb-3">
            Draft replies — {draftReplies.length}
          </h2>
          <div className="space-y-4">
            {draftReplies.map((item) => (
              <DraftReplyCard key={item.captureLogId} item={item} token={token} onDone={handleDone} />
            ))}
          </div>
        </section>
      )}
